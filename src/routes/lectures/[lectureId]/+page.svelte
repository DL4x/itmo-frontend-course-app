<script lang="ts">
    ...
    onMount(() => {
        ...
        console.log(
            `User visits lecture ${data.presentation.presentationName} with courseId = ${data.presentation.courseDocumentId}`
        );
        addProgressPresentation(
            $userStore,
            data.presentation.courseDocumentId,
...
{#if showToast}
    <div class="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm">
        <div
            class="bg-[rgba(20,30,80,0.9)] dark:bg-[rgba(10,15,40,0.95)]
...
<div class="w-[80%] mx-auto py-8">
    ...
    <div class="w-full mb-8">
        ...
        <div class="mt-4 flex md:flex-row not-md:flex-col gap-2">
            ...
            {#if summaryPromise}
                ...
                {:catch error}
                    <p class="error">
                        {error.message || 'Произошла ошибка при генерации конспекта'}
                    </p>
                {/await}
            {:else}
                <button
    ...
    <div class="w-full mb-8">
        <Card
            class="w-full max-w-none bg-[rgba(20,30,80,0.5)] dark:bg-[rgba(10,15,40,0.6)] rounded-2xl p-8 shadow-[0_0_30px_rgba(100,70,255,0.2)] backdrop-blur-md border border-[rgba(100,150,255,0.2)]"
        >
            ...
            <form class="w-full">
                ...
                {/if}
                <Button
                    class="w-full sm:w-auto bg-[#FE8A70] hover:bg-[#FE7A60] text-white font-medium py-2 px-6 rounded-lg transition-colors duration-200"
        ...
        <div class="space-y-4 w-full">
            ...
            {:else}
                {#each comments as comment (comment.id)}
                    <Card
                        class="w-full max-w-none bg-[rgba(20,30,80,0.5)] dark:bg-[rgba(10,15,40,0.6)] rounded-xl p-6 shadow-[0_0_30px_rgba(100,70,255,0.2)] backdrop-blur-md border border-[rgba(100,150,255,0.2)]"
                    >
                        <div class="flex items-start">
                            <Avatar
                                class="size-l bg-[rgba(255,255,255,0.3)] text-white transition-colors flex-shrink-0"
                            >
                            ...
                            <div class="ml-4 md:ml-6 flex-1">
                                ...
                                <P
                                    class="text-base mt-3 sm:text-lg text-white/90 dark:text-gray-200"
                                >
                                ...
                                <div class="mt-4 flex items-center space-x-4 text-sm">
                                    <button
                                        class="text-white/80 hover:text-white dark:text-gray-300 dark:hover:text-white transition-colors"
                                    >
